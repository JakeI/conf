...
elseif FORMAT == "rtf" then
  filetype = "png"
  mimetype = "image/png"
elseif FORMAT == "latex" then  -- JKI: provide latex with a proper format (tested only for graphiz)
  filetype = "eps"
  mimetype = "image/eps"
end
...
local function graphviz(code, filetype)
  -- Nice idea but I cannot get those images to scale properly
  -- if FORMAT == "latex" then -- latex use tikz and therefor allow latex within labels
  --   return pandoc.pipe("dot2tex", {"--figonly", "--texmode", "math", "--format", "tikz"}, code)
  -- else
...
function CodeBlock(block)
  ...
  -- JKI: quick and dirty fix: change output to pgf for graphviz
  -- if FORMAT == "latex" and block.classes[1] == "graphviz" then
  --   filetype = "pgf"
  --   mimetype = "text/pgf"
  -- end
  ...
  -- Make sure, the img/. directory actually exists
  --system.make_directory("img") -- for some reason this claims to be nil just relay on the user to ensure the directory exists for now

  -- Create figure name by hashing the image content
  -- local fname = pandoc.sha1(img) .. "." .. filetype
  local fname = "img/" .. pandoc.sha1(img) .. "." .. filetype
